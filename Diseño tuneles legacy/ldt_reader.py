...
class LDTReader:
    ...
    def extraer_zip_ldts(self, zip_path):
        """
        Extrae información de todos los LDTs en un ZIP
        Busca archivos con extensión .ldt, .ies, .txt o sin extensión
        """
        try:
    ...
    def obtener_info_luminaria(self, id_luminaria):
        ...
        return self.luminarias.get(id_luminaria, None)

def cargar_ldts_desde_zip(zip_path):
    """
    Función helper para cargar LDTs
    """
    try:
        reader = LDTReader(zip_path)
        luminarias = reader.extraer_zip_ldts(zip_path)

        if not luminarias:
            print(f"⚠️ No se encontraron LDTs válidos en {zip_path}")
            return None

        reader.luminarias = luminarias
        return reader

    except Exception as e:
        print(f"❌ Error cargando LDTs: {str(e)}")
        return None

def obtener_lista_luminarias_json(zip_path):
    """
    Retorna lista de luminarias como JSON para enviar al frontend
    """
    try:
        print(f"\n📚 Leyendo LDTs desde: {zip_path}")
        ...
        if not os.path.exists(zip_path):
            ...
            return {
                'success': False,
                'luminarias': [],
                'error': f'Archivo no encontrado: {zip_path}'
            }
        ...
        lista = reader.obtener_lista_luminarias()
        print(f"✓ {len(lista)} luminarias cargadas correctamente")
            ...
            'luminarias': [
                {
                    ...
                    'label': item['label'],
                    'flujo': item['flujo'],
                    'nombre': item['info'].get('nombre', ''),
                    'fabricante': item['info'].get('fabricante', ''),
                    'tipo': item['info'].get('tipo', ''),
                }
                for item in lista
            ],
            'total': len(lista)
        ...
        import traceback
        print(f"❌ Error leyendo LDTs: {str(e)}")
        print(traceback.format_exc())
        return {
            'success': False,
            'luminarias': [],
