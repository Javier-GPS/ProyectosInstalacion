...
@app.route('/tunnel')
def tunnel_index():
    """Entrada única del módulo: abre el workspace React sin mapa ni OSM."""
    return redirect('http://127.0.0.1:5173/projects')


# ══════════════════════════════════════════════════════════════════
# React workspace — project catalogue and saved tunnel studies
# ══════════════════════════════════════════════════════════════════

@app.route('/api/tunnel/projects', methods=['GET'])
def tunnel_projects_list():
    """Lista proyectos guardados para la aplicación React de túneles."""
    try:
        from modules.projects_store import list_projects
        return jsonify(list_projects())
    except Exception as exc:
        app.logger.exception('Error listando proyectos de túnel: %s', exc)
        return jsonify({'error': str(exc)}), 500


@app.route('/api/tunnel/projects', methods=['POST'])
def tunnel_projects_create():
    """Crea un proyecto de túnel en estado borrador."""
    try:
        from modules.projects_store import create_project
        payload = request.get_json(silent=True) or {}
        return jsonify(create_project(payload)), 201
    except ValueError as exc:
        return jsonify({'error': str(exc)}), 400
    except Exception as exc:
        app.logger.exception('Error creando proyecto de túnel: %s', exc)
        return jsonify({'error': str(exc)}), 500


@app.route('/api/tunnel/projects/<int:project_id>', methods=['GET'])
def tunnel_projects_get(project_id):
    """Devuelve un proyecto y su configuración guardada."""
    try:
        from modules.projects_store import get_project, mark_project_opened
        project = get_project(project_id)
        if project:
            project = mark_project_opened(project_id)
        return jsonify(project) if project else (jsonify({'error': 'Proyecto no encontrado'}), 404)
    except Exception as exc:
        app.logger.exception('Error leyendo proyecto de túnel: %s', exc)
        return jsonify({'error': str(exc)}), 500


@app.route('/api/tunnel/projects/<int:project_id>', methods=['PUT'])
def tunnel_projects_update(project_id):
    """Actualiza datos básicos, configuración y resultado del proyecto."""
    try:
        from modules.projects_store import update_project
        payload = request.get_json(silent=True) or {}
        ...
        app.logger.exception('Error actualizando proyecto de túnel: %s', exc)
        return jsonify({'error': str(exc)}), 500


@app.route('/api/tunnel/projects/<int:project_id>', methods=['DELETE'])
def tunnel_projects_delete(project_id):
    """Elimina un proyecto y sus datos guardados."""
    try:
        from modules.projects_store import delete_project
        if not delete_project(project_id):
            return jsonify({'error': 'Proyecto no encontrado'}), 404
        return jsonify({'success': True})
    except Exception as exc:
        app.logger.exception('Error eliminando proyecto de túnel: %s', exc)
        return jsonify({'error': str(exc)}), 500
...
@app.route('/api/tunnel/validate', methods=['POST'])
def tunnel_validate():
    """Valida la definición directa del túnel sin inicializar mapas ni calcular."""
    try:
        from modules.tunnel.validation import validate_tunnel_params

        data = request.get_json(silent=True) or {}
        validation = validate_tunnel_params(data)
        return jsonify(validation), 200 if validation['valid'] else 422
    except Exception as exc:
        app.logger.exception('Error validando parámetros de túnel: %s', exc)
        return jsonify({'valid': False, 'errors': [str(exc)], 'warnings': []}), 500
...
def tunnel_luminaires():
    ...
    try:
        ...
        def number_or(value, default):
            """Acepta el valor vacío que produce un input HTML y conserva el automático."""
        ...
        luminaire_params['speed_kmh']      = float(number_or(data.get('speed_kmh'), 80))
        ...
        luminaire_params['ta_design_c']      = float(number_or(data.get('ta_design_c'), 20.0))
        ...
        luminaire_params['rho_wall'] = float(number_or(data.get('rho_wall'), 0.40))
        luminaire_params['rho_ceiling'] = float(number_or(data.get('rho_ceiling'), 0.25))
        # d_min: viene del campo en LumConfigPanel, dentro de 'luminaire'
        if 'd_min' not in luminaire_params:
            luminaire_params['d_min'] = float(number_or(data.get('luminaire', {}).get('d_min'), 1.0))
        ...
        luminaire_params['height_m']     = float(number_or(data.get('height_m'), 5.5))
        luminaire_params['tunnel_shape'] = str(data.get('tunnel_shape', 'horseshoe'))
        luminaire_params['H_pared_m']    = float(number_or(data.get('H_pared_m'), 3.0))
        ...
        luminaire_params['lane_width_m'] = float(number_or(
            data.get('lane_width_m', data.get('road_width_m', data.get('width_m', 7.0))), 7.0
        ))
        ...
        if 'wall_offset_m' not in luminaire_params or luminaire_params.get('wall_offset_m') in (None, ''):
            luminaire_params['wall_offset_m'] = number_or(data.get('luminaire', {}).get('wall_offset_m'), 0.30)
        road_width_m     = float(number_or(data.get('road_width_m', data.get('width_m', 7.0)), 7.0))
...
def tunnel_report():
    ...
    try:
        ...
        # Reutilizar el resultado visible en la aplicación cuando existe.
        # Así el informe documenta exactamente la última ejecución y no vuelve
        # a calcular CIE 88 con una configuración distinta al descargarlo.
        cached_cie_result = data.get('cie_result')
        result = (
            dict(cached_cie_result)
            if isinstance(cached_cie_result, dict) and cached_cie_result.get('success')
            else run_tunnel_calculation(data)
        )
        if not result.get('success'):
            return jsonify(result), 422

        cached_control = data.get('control_result')
        if isinstance(cached_control, dict) and isinstance(cached_control.get('control'), dict):
            result['control'] = cached_control['control']
