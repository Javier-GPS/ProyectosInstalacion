...
def run_tunnel_calculation(params: dict) -> dict:
    ...
    try:
        ...
        # Los formularios web mantienen los inputs vacíos como cadenas. Tras
        # la validación no deben llegar nunca a float()/int() sin normalizar:
        # así un campo opcional vacío conserva su valor automático y un campo
        # obligatorio recibe el mensaje de validación correspondiente.
        def numeric_or(key, default):
            value = params.get(key, default)
            if value in (None, "") or (isinstance(value, str) and not value.strip()):
                return default
            return value

        length_m        = float(numeric_or("length_m", 300))
        speed_kmh       = float(numeric_or("speed_kmh", 80))
        traffic_veh_h   = int(numeric_or("traffic_veh_h", 500))
        num_lanes       = max(1, int(numeric_or("num_lanes", 1)))
        gradient_pct    = float(numeric_or("gradient_pct", 0.0))
        ...
        wall_refl       = float(numeric_or("wall_reflectance", 0.4))
        ...
        reaction_time_s = float(numeric_or("t_reaction", 2.5))
        ...
        n_steps         = int(numeric_or("n_steps", 4))
        ...
        night_normal_override = optional_float("night_normal_luminance_cd_m2")
        if night_normal_override is not None:
            L_night_normal = max(0.0, float(night_normal_override))
        else:
            L_night_normal = Lin

        night_reduced_override = optional_float("night_reduced_luminance_cd_m2")
        if night_reduced_override is None:
            night_reduced_override = optional_float("night_luminance_cd_m2")
        if night_reduced_override is not None:
            L_night_reduced = max(0.0, float(night_reduced_override))
        else:
            external_road_luminance = optional_float("external_road_luminance_cd_m2")
        ...
        # 12. PLAN DE CONTROL
        n_tr_groups = int(numeric_or("n_transition_groups", 2))
        ...
        control_plan = build_control_plan(
            ...
            driver_min_dim_pct=float(numeric_or("driver_min_dim_pct", 0.1)),
