"""GIS — SQLAlchemy models for SALVI GIS data in PostgreSQL.

All tables are prefixed ``gis_`` to avoid collisions with LuxStudio's
own domain tables (``projects``, ``users``, …).
"""
...
# ── helper: naive-or-aware timestamp ──────────────────────────────────────────
def _utcnow() -> datetime:
    return datetime.now(timezone.utc)


# ── Zone (the core GIS entity) ────────────────────────────────────────────────
class GisZone(Base):
    ...
    config: Mapped["GisZoneConfig | None"] = relationship(
        "GisZoneConfig", back_populates="zone", uselist=False,
        cascade="all, delete-orphan"
    )
    osm_data: Mapped["GisZoneOsmData | None"] = relationship(
        "GisZoneOsmData", back_populates="zone", uselist=False,
        cascade="all, delete-orphan"
    )
    trees: Mapped["GisZoneTrees | None"] = relationship(
        "GisZoneTrees", back_populates="zone", uselist=False,
        cascade="all, delete-orphan"
    )
...
class GisZoneConfig(Base):
    ...
    zone_id: Mapped[str] = mapped_column(
        String(50), ForeignKey("gis_zones.id", ondelete="CASCADE"),
        primary_key=True
    )
...
class GisLuminaire(Base):
    ...
    zone_id: Mapped[str] = mapped_column(
        String(50), ForeignKey("gis_zones.id", ondelete="CASCADE"), nullable=False
    )
...
# ── Inventory luminaires (field inventory) ────────────────────────────────────
...
# ── Photometric results (imported from LuxStudio) ─────────────────────────────
class GisPhotometricResult(Base):
    __tablename__ = "gis_photometric_results"
    __table_args__ = (
        UniqueConstraint("zone_id", "match_key", name="uq_gis_photo_zone_match"),
    )
...
# ── Project UI config (per-project key-value JSON) ────────────────────────────
class GisProjectUiConfig(Base):
    __tablename__ = "gis_project_ui_config"
    __table_args__ = (
        PrimaryKeyConstraint("project_id", "config_key", name="pk_gis_ui_config"),
    )
...
def ensure_gis_tables() -> None:
    """Create all GIS tables if they don't exist yet (idempotent)."""
